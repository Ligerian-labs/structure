...
- Dependency direction (no cycles): config ← observability ← everything; domain ← cqrs ← eventsourcing ← sql adapters ← viewmodel; migrations and auth are standalone (their Effect dependency aside); runtime ← http/cli; ai and mcp are leaves.
