...
const program = Effect.gen(function* () {
  ...
  const account = yield* store.get("acc-1"); // Effect<A, NotFound | PersistenceError>
...
Every store operation can fail with `PersistenceError` for SQL or row-codec failures. `get` and `patch` also retain `NotFound`. Hydration propagates persistence failures. Use `Effect.catchTag` at an application recovery boundary; see the [shared error contract](../eventsourcing/README.md#persistence-failures).
