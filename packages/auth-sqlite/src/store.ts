...
/**
 * Decodes persisted flow context defensively: the column is written only by
 * validated `beginOAuth` calls, but a corrupted row must never crash a
 * callback — it degrades to no context rather than failing the sign-in.
 */
