...
/**
 * Header names the drivers generate from the message itself. A custom header
 * may never carry one of them: it would forge the envelope (`Bcc`, `From`)
 * or displace the MIME structure (`Content-Type`) the renderer declares.
 */
...
export const isReservedHeaderName = (name: string): boolean =>
  RESERVED_HEADER_NAMES.has(name.trim().toLowerCase());
...
/**
 * Custom transport headers. Names and values are validated against CRLF
 * injection, and the generated envelope and MIME header names are refused
 * (a record-level filter: a key the key schema excludes is dropped, but a
 * reserved name is well-formed, so it reaches the filter and fails it).
 */
export const EmailHeaders = Schema.Record({ key: headerName, value: headerValue }).pipe(
  Schema.filter(
    (headers) => {
      const reserved = Object.keys(headers).filter(isReservedHeaderName);
      return reserved.length === 0
        ? true
        : `header ${reserved.join(", ")} is generated by the driver and cannot be overridden`;
    },
    { identifier: "EmailHeaders" },
  ),
);
