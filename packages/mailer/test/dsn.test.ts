...
/** A cleartext relay that advertises STARTTLS: AUTH must only happen after the upgrade. */
const startStarttlsRelay = async (): Promise<{
  readonly port: number;
  /** Commands seen before the TLS upgrade (must never include AUTH). */
  readonly plaintextCommands: Array<string>;
  /** Commands seen after the TLS upgrade. */
  readonly secureCommands: Array<string>;
  readonly authPlainTokens: Array<string>;
  readonly close: () => Promise<void>;
}> => {
  const plaintextCommands: Array<string> = [];
  const secureCommands: Array<string> = [];
  const authPlainTokens: Array<string> = [];
  const server = net.createServer((raw) => {
    let active: net.Socket = raw;
    let secure = false;
    let inData = false;
    let buffer = "";
    const handle = (chunk: Buffer): void => {
      ...
      if (inData) {
        ...
        for (;;) {
          ...
          buffer = buffer.slice(index + 5);
          inData = false;
          active.write("250 queued\r\n");
        }
        return;
      }
      (secure ? secureCommands : plaintextCommands).push(text.trim());
      const upper = text.trim().toUpperCase();
      if (upper.startsWith("EHLO")) {
        active.write("250-relay greets you\r\n250-STARTTLS\r\n250 AUTH PLAIN LOGIN\r\n");
      } else if (upper === "STARTTLS") {
        active.write("220 go ahead\r\n");
        raw.removeAllListeners("data");
        const upgraded = new tls.TLSSocket(raw, {
          isServer: true,
          key: fixture("smtp-test-key.pem"),
          cert: fixture("smtp-test-cert.pem"),
        });
        active = upgraded;
        secure = true;
        ...
        inData = true;
        buffer = "";
        active.write("354 end with <CRLF>.<CRLF>\r\n");
      } else if (upper === "QUIT") {
        active.write("221 bye\r\n");
        active.end();
      }
    };
    raw.on("data", handle);
    raw.write("220 starttls.example ESMTP ready\r\n");
...
describe("dsn-built driver on the wire: smtp:// upgrades with STARTTLS before AUTH", () => {
  let relay: Awaited<ReturnType<typeof startStarttlsRelay>> | undefined;
  afterAll(async () => {
    if (relay !== undefined) await relay.close();
  });

  test("authenticates only after the TLS upgrade, credentials never in the clear", async () => {
    relay = await startStarttlsRelay();
    const driver = await run(
      driverFromDsn(`smtp://mailer:hunter2@127.0.0.1:${relay.port}?verify_peer=false`),
    );
    expect(driver.name).toBe("smtp");
    await run(
      driver.send({
        from: { email: "app@example.com" },
        to: [{ email: "ada@example.com" }],
        subject: "starttls dsn",
        text: "after upgrade",
      }),
    );
    // The cleartext phase carries EHLO and STARTTLS only; AUTH, envelope, and
    // DATA all appear on the upgraded session.
    expect(relay.plaintextCommands).toEqual(["EHLO structure-mailer", "STARTTLS"]);
    expect(relay.authPlainTokens).toHaveLength(1);
    const decoded = Buffer.from(relay.authPlainTokens[0] ?? "", "base64").toString("utf8");
    expect(decoded).toBe("\u0000mailer\u0000hunter2");
