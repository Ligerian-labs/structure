...
Taxonomy mapping validates fields, not just `_tag`; platform errors use their constructors. The CQRS bridge maps typed errors without flattening compound causes. Any defect produces a safe 500, and cancellation remains interrupted even when another failure is present. Missing static files still fall through to 404; unexpected filesystem failures produce 500.
