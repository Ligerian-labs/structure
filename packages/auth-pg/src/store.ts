...
interface PasskeyRow {
  ...
  readonly counter: number | bigint | string;
        ...
        await sql`
          ...
          VALUES
            (${record.tenantId}, ${record.userId}, ${record.secretBase32},
             ${record.confirmed}, ${JSON.stringify(record.recoveryCodeHashes)},
    ...
    confirmTotp: (tenantId, userId, recoveryCodeHashes, now) =>
      read("confirm-totp", async () => {
        const rows = await sql<TotpRow[]>`
          ...
          SET confirmed = TRUE, recovery_code_hashes = ${JSON.stringify(recoveryCodeHashes)},
              failed_attempts = 0, locked_until = NULL, enrolled_at = ${now.toISOString()}
          WHERE tenant_id = ${tenantId} AND user_id = ${userId} AND confirmed = FALSE
