...
`position` is drawn from a sequence at insert time, inside the append transaction, so two concurrent appends could commit in the opposite order of their positions; a projection polling in between would checkpoint past a position whose transaction had not committed yet and never deliver that event (it is committed, visible to `read`, and replayed by a rebuild — the live view and the rebuilt view diverge silently). `EventStore.append` and `appendWithOutbox` therefore take a transaction-scoped advisory lock (`pg_advisory_xact_lock(0x5f455654, hashtext(<events table>))`) after the version check and before the first insert; Postgres releases it as part of the commit, after the transaction is visible to new snapshots, so positions become visible in the order they were drawn and a reader that has seen position N has seen every committed event below it. Only the inserts and the commit are serialized, never the version check or the caller's own work, and an empty (version-checked) append takes no lock. One consequence to plan around: an append nested inside a caller's own `withTransaction` holds the lock until the *caller's* commit, so a caller that keeps its transaction open after appending blocks every other append to that events table for as long as it stays open — append last, and keep such transactions short. The lock has no timeout of its own; a stalled writer is a stalled store until its transaction ends.

Measured on a local Postgres 17 (two-event appends to distinct streams): a single writer is unchanged (about 2 ms per append); the serialized section costs about 1.6 ms per append and is a hard throughput ceiling of roughly 600 appends per second whatever the number of writers, so sixteen concurrent writers go from about 2,600 to about 600 appends per second with p95 latency rising from about 10 ms to about 33 ms, and thirty-two writers see a p95 of about 69 ms, pure queueing. Keep that advisory key for this package; `HistoryImporter` locks the events table exclusively and composes with it.
