...
- `makeTotp` requires `secret` (`Redacted`, the instance secret, at least 32 characters or it throws at boot): the TOTP secret and the recovery codes are sealed at rest under keys derived from it. Expose `resetSecondFactor` only on an operator surface (a CLI command, a superadmin route), audited with the operator as `actor`.
