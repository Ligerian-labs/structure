...
3. **Run the relay** in a worker process: `OutboxRelay.run` claims due entries → publishes → settles; exponential backoff with jitter; after `maxAttempts` (default 5) entries dead-letter and keep the last error. Backoff is durable — the relay persists the next eligible time (`markFailed`), so a restart does not make a backing-off message immediately due. `OutboxMessage.availableAt` schedules the first attempt (durable delayed delivery); `OutboxRelay.drain` empties the queue once (tests, shutdown). Delivery ownership is claim-based: each batch is leased (`claim(limit, lease)`, default lease 30s; PostgreSQL uses `FOR UPDATE SKIP LOCKED`), so several relays may run concurrently without double delivery within a lease; a crashed relay's lease expires and another relay recovers the entry (at-least-once).
