...
4. **Wire `OutboxRelay.run`** in a worker (or every instance, if cheap): claim → publish → settle, exponential backoff with jitter, dead letters after `maxAttempts` with the last error kept. Delivery ownership is claim-based — each batch is leased (`OutboxRelayOptions.lease`, default 30s; PostgreSQL claims via `FOR UPDATE SKIP LOCKED`), so several relays may run concurrently without double delivery within a lease, and a crashed relay's lease expiry makes its entries claimable again (at-least-once). Backoff and `availableAt` scheduling are durable in the SQL adapters (survive restarts); requeue dead letters with `Outbox.replay([id])`.
