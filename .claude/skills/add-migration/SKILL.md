...
import { defineMigration } from "@structure/migrations";
...
For a new view-model table use `ViewModel.migration(def, id)` from `@structure/viewmodel` instead of hand-written DDL.
...
5. **Run it** where the deployment policy allows: `<app> migrations up` (CLI group via `migrationsCommand(set)` from `@structure/migrations/cli`), or `layer(set)` in the one process permitted to migrate. `migrations status` shows applied/pending.
