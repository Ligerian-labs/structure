...
- Auth schema: on Postgres, put `auth-pg`'s frozen `migration(id)` and each later package migration, including `passkeyMetadataMigration(nextId)`, in the application's single `makeSet`. Existing applications add new package migrations under unused ids and never renumber the applied initial migration. `auth-pg.migrate(sql)` (Bun `SQL`) and `auth-sqlite.migrate` are the all-in-one alternatives for apps without a set. Either way, one migration owner; serving processes construct `makeAuthStore` without migrating. Stores never create tables.
- `eventsourcing-pg.migrate` (run by its `layer` at build) also creates the `idempotency` table behind the cqrs `IdempotencyStore`; the same single-owner rule applies when several instances share the database. Its schema is exposed as versioned steps (`migrations`, rev 1 the tables, rev 2 the generated `partition` column and index on `events`): record each rev as its own entry in your set if you keep one. Rev 2 **rewrites the `events` table once under an exclusive lock** (seconds to minutes on a large store) — apply it in a maintenance window before deploying the first version that calls it, or accept the pause at that boot.
...
- **Erasing a stream (GDPR/retention).** Use `StreamEraser.eraseStream({ streamName, expectedVersion, reason })` — never hand-written `DELETE`s against the events table. Event content is destroyed in place (`Erased` tombstones keep positions, so projection checkpoints stay valid and `readAll` stays gap-free), the stream's snapshot goes with it, and the stream is pinned: every later append fails `ConcurrencyConflict`. Idempotent — retrying with the same `expectedVersion` is safe. Erasure does not retract already-published outbox messages or already-projected read-model rows; drop those through their own delete paths, then rebuild affected views (tombstones are skipped, so erased content never comes back).
