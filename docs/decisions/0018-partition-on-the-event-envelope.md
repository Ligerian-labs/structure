...
Easier: an exporter, a replica, or any caller driving `readAll` reads one subset of the feed in global order with the global positions, so it can checkpoint exactly as an unfiltered reader does (`Projection` itself stays partition-agnostic in this decision; a partition-scoped projection is a follow-up); a database policy or a partitioned table can be built on a real column; a replicated copy can carry its provenance; an application can attach envelope-level facts (a delegating principal, say) without a payload convention. Nothing changes for an application that never sets a partition: every field and option is optional, `readAll` without a filter runs the same query plus one column nobody reads, and existing frozen histories checksum identically.
